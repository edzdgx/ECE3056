...
pfn_t free_frame(void) {
    ...
    if (frame_table[victim_pfn].mapped == 1) {
        // 1. look for current entry
        vpn_t vpn = frame_table[victim_pfn].vpn;
        pte_t* entry = (pte_t*) (mem + (frame_table[victim_pfn].process)->saved_ptbr * PAGE_SIZE) + vpn; // current entry

        // 2. if dirty, writeback and swap_write()
        if (entry->dirty == 1) {
            stats.writebacks++;

            void* paddr = (void*) (mem + victim_pfn * PAGE_SIZE);
            // swap_write(pte_t *pte, void *src)
            // swap pte with physical addresss
            swap_write(entry, paddr);
            entry->dirty = 0;
        }

        // 3. mark original as invalid
        entry->valid = 0;
